...
if ( !empty($_POST['new']) && !empty($_POST['new_confirm']) ){

    if ( $_POST['new'] == $_POST['new_confirm'] ){

        $mysqlClient->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sqlQuery = 'UPDATE administrateur SET MDPAdmin = :MDPAdmin WHERE MailAdmin= :MailAdmin';

        $UpdateAdmin = $mysqlClient->prepare($sqlQuery);

        list($a, $b) = explode('@', $_SESSION['LOGGED_USER']);
        $salt = $a;
        $digest = crypt($_POST['new'] , '_J9..rasm'); 
...
<html lang="fr">
	<head>
		...
		<title>Skyincap - Validation</title>
				...
				<div class="u-top-column-t-long">
					...
					<div class="u-top-text-long">
							<button style="width:100% ; height: 40px; background-color:#4861AD;" type="submit">
								<a style="color: white;" href="connexion.php"> Reconnectez vous avec vos nouveaux identifiants </a>
							</button>
							<?php
        ...
        <div class="section2">
            <div class="u-container">
				<div class="u-top-column-t-long">
					...
					<div class="u-top-text-long">
							<div class="echec_bouton" style="width:100% ; height: 40px; color:#CC3F44;	border: 0; margin-bottom: 40px; text-align: left; font-weight: bold; font-size: 22px; ">Les deux mots de passe sont différents</div>
							<button style="width:100% ; height: 40px; background-color:#4861AD;" type="submit">
								<a style="color: black;" href="changement.php"> Réessayer </a>
							</button>
					</div>
				</div>
            </div>
