...
session_start();
	...
	$uploadDirectory = "../asset/avatar/";
	$errors = []; // Store errors here
	$fileExtensionsAllowed = ['jpeg','jpg','png','blg']; // These will be the only file extensions allowed 
	...
	if (isset($_POST['submit']) && isset($_POST['cle']) && isset($_POST['email_avatar'])) {

	if (file_exists($uploadPath)) {
		$_SESSION['nom_image'] = 1;
	}

	else {

        if (($_POST['cle']) <> "Y09BIkup7clz9Gm") {
            $_SESSION['nom_image'] = 1;
	    }

        else {
            $_SESSION['nom_image'] = 2;
        }
...
if ($_SESSION['nom_image'] == 2){

	$sqlQuery = 'INSERT INTO avatar(MailAvatar,Image_Avatar) VALUES
	(:MailAvatar,:Image_Avatar)';

	$insertActu = $mysqlClient->prepare($sqlQuery);
	$insertActu->execute([
    'MailAvatar' => $_POST['email_avatar'],
    	...
    	border-radius: 5px;">
					...
					<aside id="modal-language" class="u-modal" style="display: none;">
						<div class="u-modal-wrapper js-modal-stop">
							...
							<div class="u-modal-item">
								...
								<a href="../eng/accueil.php" class="header-boutton-language-struc u-rebond-fleche" >
			...
			<aside id="modal-accessibility-info" class="u-modal" style="display: none;">
				<div class="u-modal-wrapper js-modal-stop">
					...
					<div class="modal-body">
						<h2>Pourquoi ce bandeau d'accessibilitée ? <?php echo $textetrue ?></h2>
				...
				<?php if ($_SESSION['nom_image'] == 2): ?> 
					...
					<div class="u-top-text-long">
							<button style="width:100% ; height: 40px; background-color:#4861AD;" type="submit">
								<a style="color: white;" href="../index.php">   Retour à la page d'accueil du site </a>
							</button>
					</div>
				<?php else: ?> 
					<h1 class="no-margin"> <span class="u-titre-principal-color"> ERREUR ! Le nom d'image est déjà utilisé ou la clé rentrée n'est pas correcte </span></h1>
					<div class="u-top-text-long">
							<button style="width:100% ; height: 40px; background-color:#4861AD;" type="submit">
								<a style="color: white;" href="EnregistrementAvatar.php"> Retour à la page précédente </a>
							</button>
					</div>
				<?php endif; ?>
                </div>
